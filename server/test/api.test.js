...
describe("SummitScene API", function () {
  ...
  it("should let an admin delete analytics for one source attribution", async () => {
    const attributionKey = `source:delete-test-${testRunId}`;
    process.env.ADMIN_EMAILS = testEmail;
    ...
    try {
      await AnalyticsEvent.create({
        ...
        attributionName: "Delete Test Source",
        sessionId: `delete-source-session-${testRunId}`,
        metadata: { testRunId },
      });

      const res = await request(app)
        .delete(`/api/analytics/attribution/${encodeURIComponent(attributionKey)}`)
        .set("Authorization", `Bearer ${authToken}`);
      ...
      const remaining = await AnalyticsEvent.countDocuments({ attributionKey });
      expect(remaining).to.equal(0);
    } finally {
      await AnalyticsEvent.deleteMany({ attributionKey });
      process.env.ADMIN_EMAILS = originalAdminEmails;
    }
  });

  it("should skip analytics generated by admin accounts", async () => {
    const eventId = await ensureSharedTestEvent();
    process.env.ADMIN_EMAILS = testEmail;
    ...
    try {
      const res = await request(app)
        ...
        .send({
          type: "event_view",
          eventId,
          sessionId: `admin-session-${testRunId}`,
          metadata: { testRunId, adminSkipped: true },
        });

      expect(res.status).to.equal(202);
      expect(res.body.skipped).to.equal("admin");

      const saved = await AnalyticsEvent.findOne({
        "metadata.adminSkipped": true,
        "metadata.testRunId": testRunId,
      });
  ...
  it("should let an admin delete an event import source", async () => {
    process.env.ADMIN_EMAILS = testEmail;
    const sourceUrl = `https://example.com/source-delete-${testRunId}`;

    try {
      await EventSource.deleteOne({ url: sourceUrl });
      const source = await EventSource.create({
        name: `Deleted Source ${testRunId}`,
        url: sourceUrl,
        town: "Banff",
        sourceType: "html",
      });
      ...
      const res = await request(app)
        .delete(`/api/event-import/sources/${source._id}`)
        .set("Authorization", `Bearer ${authToken}`);

      expect(res.status).to.equal(200);
      expect(res.body.sourceName).to.equal(`Deleted Source ${testRunId}`);

      const saved = await EventSource.findById(source._id);
      expect(saved).to.equal(null);
    } finally {
      await EventSource.deleteOne({ url: sourceUrl });
      process.env.ADMIN_EMAILS = originalAdminEmails;
