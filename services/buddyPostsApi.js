...
export async function fetchBuddyPostById(postId, token) {
  try {
    const res = await fetchWithTimeout(`${API_BASE_URL}/api/buddy-posts/${postId}`, {
      method: "GET",
      headers: buildHeaders(token),
    ...
    if (!res.ok) {
      throw new Error(
        data.error || data.message || `Failed to load buddy post (${res.status})`
      );
    }
    ...
    const normalizedError = normalizeBuddyPostError(
      error,
      "Buddy post request timed out. Check the backend and try again."
    );
    console.warn("fetchBuddyPostById issue:", normalizedError.message);
    throw toUserFriendlyError(
      normalizedError,
      "We couldn't load that community post right now. Please try again."
    );
  }
}
